...
int	PhoneBook::Print_Cell(std::string input)
{
	...
	print_table();
	tab.push_back(input);
	tab.push_back(this->profile[position].get_first_name());
	tab.push_back(this->profile[position].get_last_name());
	tab.push_back(this->profile[position].get_nickname());
